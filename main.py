...
from utils.features.prepare_for_training import prepare_for_training1
...
class LogisticRegression:
    def __init__(self, data, labels, polynomial_degree=0, sinusoid_degree=0, normalize_data=False):
        ...
        初始化参数矩阵
        """
        (data_processed, feature_mean, features_deviation) = prepare_for_training1 (data, polynomial_degree,sinusoid_degree, False)
        self.data = data_processed
        self.labels = labels
        self.unique_labels = np.unique(labels)
        self.features_mean = feature_mean
    ...
    def gradient_descent(data, labels, current_initial_theta, max_iterations):
        ...
        if not result.success:
            raise ArithmeticError('Could not train the model.' + result.message)
    ...
    def predict(self, data):
        num_examples = data.shape[0]
        data_processed = prepare_for_training1 (data, self.polynomial_degree, self.sinusoid_degree, self.normalize_data)[
            0]
